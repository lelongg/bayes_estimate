...
use na::{RealField, allocator::Allocator, DefaultAllocator, Dim, VectorN};
use bayes_filter::models::{LinearEstimator, KalmanState, LinearPredictor, LinearPredictModel, AdditiveNoise, AdditiveCorrelatedNoise,
...
impl<N: RealField, D: Dim, ZD: Dim, ZQD : Dim> LinearObservationCorrelated<N, D, ZD, ZQD> for NullState<N, D>
where
    DefaultAllocator: Allocator<N, D, D> + Allocator<N, ZD, ZD> + Allocator<N, ZD, D> + Allocator<N, D, ZD> + Allocator<N, ZD, ZQD>
...
impl<N: RealField, D: Dim, ZD: Dim, ZQD : Dim> LinearObservationUncorrelated<N, D, ZD, ZQD> for NullState<N, D>
    where
        DefaultAllocator: Allocator<N, D, D> + Allocator<N, ZD, ZD> + Allocator<N, ZD, D> + Allocator<N, D, ZD> + Allocator<N, ZD, ZQD>
        + Allocator<N, D> + Allocator<N, ZD> + Allocator<N, ZQD>
{
