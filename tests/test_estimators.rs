...
use na::{Dim, Dynamic, RealField};
use na::{DVector, VectorN};
use na::{Matrix, Matrix1, Matrix1x2, Matrix2, Matrix2x1, MatrixMN, Vector1, Vector2};
...
#[test]
fn test_unscented() {
    test_estimator(&mut FatUnscentedState::new_zero(U2));
    test_estimator(&mut FatUnscentedState::new_zero(Dynamic::new(2)));
}
...
fn test_sir() {
    ...
    {   // Vector2
        ...
        test_estimator(&mut FatSampleState {
...
fn test_estimator<D: Dim>(est: &mut dyn FatEstimator<D>)
    where
